...
<form action="{{ route('admin.menu-items.update', $menuItem) }}" method="POST" enctype="multipart/form-data">@csrf @method('PUT')
    ...
    <div class="mb-3"><label class="form-label">Image</label>@if($menuItem->image)<div class="mb-2"><img src="@media($menuItem->image)" height="80" class="rounded object-fit-cover" alt="{{ $menuItem->name }}" onerror="this.src='{{ asset('images/logo-icon.png') }}'"></div>@endif<input type="file" name="image" class="form-control" accept="image/*"></div>
